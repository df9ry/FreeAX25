		...
		FreeAX25::UniquePointerDict<FreeAX25::Setting>& settings)
{
	...
	for (uint32_t i = 0; i < nodeList->getLength(); ++i) {
		...
		settings.insertNew(name, new FreeAX25::Setting(name, value));
	} // end for //
}
		...
		FreeAX25::UniquePointerDict<FreeAX25::Plugin>& plugins)
{
	...
	for (uint32_t i = 0; i < nodeList->getLength(); ++i) {
		...
		auto plugin = new FreeAX25::Plugin(name, file);
		...
		FreeAX25::UniquePointerDict<FreeAX25::Instance>& instances)
{
	...
	for (uint32_t i = 0; i < nodeList->getLength(); ++i) {
		...
		auto instance = new FreeAX25::Instance(name);
	    ...
	    { // Get client endpoints:
			...
			for (uint32_t i = 0; i < nodeList->getLength(); ++i) {
				...
				auto endpoint = new FreeAX25::ClientEndPoint(name, url);
	    ...
	    { // Get server endpoints:
			...
			for (uint32_t i = 0; i < nodeList->getLength(); ++i) {
				...
				auto endpoint = new FreeAX25::ServerEndPoint(name, url);
